...
# random_state=None, max_features=None, alpha=0.9, verbose=0, max_leaf_nodes=None, w
# arm_start=False, validation_fraction=0.1, n_iter_no_change=None, tol=0.0001, ccp_alpha=0.0)
...
if rank == 0:
    X_TRAIN0, X_TEST0, Y_TRAIN, Y_TEST = train_test_split(X, Y, random_state=102, test_size=0.2, shuffle=True)
else:
    X_TRAIN0, X_TEST0, Y_TRAIN, Y_TEST = None, None, None, None
X_TRAIN0 = comm.bcast(X_TRAIN0, root=0)
X_TEST0 = comm.bcast(X_TEST0, root=0)
Y_TRAIN = comm.bcast(Y_TRAIN, root=0)
Y_TEST = comm.bcast(Y_TEST, root=0)


#for i in range(len(fnamess)):
#    if i%size==rank:
#        os.mkdir(str(i))
#        os.chdir(str(i))
#        with open('log','w') as fp:
#            X_TRAIN = X_TRAIN0[fnamess[i]]
#            X_TEST = X_TEST0[fnamess[i]]
#            fp.write(str(fnamess[i])+'\n')
...
if rank == 0:
    ...
    while True:
        status = MPI.Status()
        recvnum = comm.recv(source=MPI.ANY_SOURCE, tag=MPI.ANY_TAG, status=status)
        if status.Get_tag() == 1:
            if curn < ncoms:
                comm.send(curn, dest=status.Get_source(), tag=1)
                curn = curn+1

        elif status.Get_tag() == 2:
            count = count+1
            comm.send(0, dest=status.Get_source(), tag=2)
            if count == ncoms:
                break
    for i in range(1, size):
        comm.send(0, dest=i, tag=3)
    ...
    while True:
        status = MPI.Status()
        recvnum = comm.recv(source=0, tag=MPI.ANY_TAG, status=status)
        if status.Get_tag() == 1:
            X_TRAIN = X_TRAIN0[fnamess[recvnum]]
            X_TEST = X_TEST0[fnamess[recvnum]]
            train(X_TRAIN, X_TEST, Y_TRAIN, Y_TEST, rank, recvnum, fnamess[recvnum])
