...
for name in names:
    ...
    path=surfname+'_'+site+'/'+Oid+'_'+Mid+'/fs'
    ...
    isf=read(surfname+'_'+site+'/'+Oid+'_'+Mid+'/is/OUTCAR')
    e_is=isf.get_potential_energy()
    fsf=read(surfname+'_'+site+'/'+Oid+'_'+Mid+'/fs/OUTCAR')
    e_fs=fsf.get_potential_energy()
    de=e_fs-e_is

    if res[0]==int(Mid) and d_o<0.5 and d_m<0.5 and disom<4.0 and de<0.5:
        fp1.write('{0:<50s} {1:>5d} {2:>5d} {3:>5d} {4:>20.6f}\n'.format(surfname,int(site),int(Oid),int(Mid),float(ea_gbdt)))
        fp2.write('{0:>50s} {1:>20.5f} {2:>20.5f} {3:>20.5f} {4:>20.5f}\n'.format(surfname+'_'+site+'/'+Oid+'_'+Mid,d_o,d_m,disom,de))
